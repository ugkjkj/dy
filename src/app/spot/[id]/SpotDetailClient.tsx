...
import { getSpotById, getPosts, fetchWeather } from "@/lib/static-data";
...
export default function SpotDetailClient({ spotId }: { spotId: string }) {
  ...
  useEffect(() => {
    ...
    fetchWeather(spotData.latitude, spotData.longitude).then((w) => {
      if (w) setWeather(w as WeatherData);
    });
